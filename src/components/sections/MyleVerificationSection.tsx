...
export interface MyleVerificationStep {
  /** Which built-in drawing heads the card. */
  illustration: string;
  title: string;
  description: string;
}

export interface MyleVerificationSettings {
  badgeText: string;
  heading: string;
  steps: MyleVerificationStep[];
  ctaLabel: string;
  ctaHref: string;
}

/**
 * The illustrations stay in code: they're hand-drawn vector art tuned to this
 * layout, not content. A step picks one by name, so steps can be rewritten and
 * reordered from the customizer without anyone editing SVG paths.
 */
const ILLUSTRATIONS: Record<string, React.ReactNode> = {
  qr: (
    <svg
      viewBox="0 0 200 220"
      className="w-full h-full max-h-44 stroke-foreground fill-none"
      strokeWidth="2.5"
      strokeLinecap="round"
...
const FALLBACK_STEPS: MyleVerificationStep[] = [
  {
    illustration: "qr",
    title: "FIND QR CODE",
    description:
...
export function MyleVerificationSection({
  settings,
}: { settings?: MyleVerificationSettings } = {}) {
  const steps =
    settings?.steps && settings.steps.length > 0 ? settings.steps : FALLBACK_STEPS;
  const ctaLabel = settings?.ctaLabel ?? "Authenticate MYLE Product at ac.mylevape.com";
  const ctaHref = settings?.ctaHref ?? "https://ac.mylevape.com";
      ...
      <div className="bg-card border border-border/60 rounded-[2.5rem] p-6 sm:p-10 lg:p-14 relative overflow-hidden shadow-md transition-all duration-300">
        ...
        <div className="text-center mb-10 sm:mb-12 space-y-3">
          <div className="inline-flex items-center gap-2 bg-primary/10 border border-primary/20 text-primary text-[10px] sm:text-xs font-black uppercase tracking-[0.25em] px-4 py-1.5 rounded-full">
            ...
            <span>{settings?.badgeText || "Official UAE Verification"}</span>
          </div>

          <h2 className="text-2xl sm:text-4xl lg:text-5xl font-serif font-black tracking-[0.2em] uppercase text-foreground">
            {settings?.heading || "ANTI-COUNTERFEIT SYSTEM"}
          </h2>
        </div>

        {/* White vertical cards, one per step */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 sm:gap-8 items-stretch">
          {steps.map((step, index) => (
            <div
              key={index}
              className="bg-card border border-border/80 hover:border-primary/50 rounded-3xl p-7 sm:p-8 flex flex-col items-center text-center justify-between transition-all duration-300 shadow-sm hover:shadow-xl hover:-translate-y-1.5 group"
            >
              <div className="w-full h-48 sm:h-52 flex items-center justify-center my-2 p-3 bg-muted/20 rounded-2xl border border-border/40 group-hover:scale-105 transition-transform duration-300">
                {ILLUSTRATIONS[step.illustration] ?? ILLUSTRATIONS.qr}
              </div>
