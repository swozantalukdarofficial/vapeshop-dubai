...
async function run() {
  console.log("Fetching Metaobject Definition for juul_feature...");
  const getDefsQuery = `
    query {
  ...
  if (!existing) {
    console.error("juul_feature Metaobject Definition not found");
    return;
  }

  const defId = existing.node.id;
  console.log("Found Metaobject Definition ID:", defId);

  // Let's test adding a new field definition 'description_rich' of type 'rich_text_field' or updating fieldDefinitions
  ...
  // First let's check fieldDefinitions operation input options by trying create for description_rich
  const updateInput = {
    fieldDefinitions: [
      {
        create: {
